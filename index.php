...
if ( !class_exists( 'Woo_Discounts_Plus_Plugin' ) ) {
	...
	class Woo_Discounts_Plus_Plugin {
		var $wdp_dir;
		var $wdp_pro;
		var $opts = 6;
		var $discount_love;
		...
		var $watch_tutorial = 'http://www.websitedesignwebsitedevelopment.com/wdp';
		...
		public function __construct() {
			...
			$this->wdp_dir = plugin_dir_path(__FILE__) ;
			$pro_class = $this->wdp_dir.'classes/wdp_pro.php';
			$this->wdp_pro = file_exists($pro_class);
			if($this->wdp_pro){
			...
			$this->settings_tabs = array(
				'plus_discount' => __( 'Discounts Plus'.($this->wdp_pro?'+':''), 'wc_plus_discount' )
			);
		...
		protected function get_discounted_coeff( $product_id, $quantity ) {
			...
			for ( $i = 1; $i < $this->opts; $i++ ) {
				array_push( $q, get_post_meta( $product_id, "plus_discount_quantity_$i", true ) );
				if ( get_option( 'woocommerce_discount_type', '' ) == 'flat' ) {
					array_push( $d, get_post_meta( $product_id, "plus_discount_discount_flat_$i", true ) ? get_post_meta( $product_id, "plus_discount_discount_flat_$i", true ) : 0.0 );
				} else {
					array_push( $d, get_post_meta( $product_id, "plus_discount_discount_$i", true ) ? get_post_meta( $product_id, "plus_discount_discount_$i", true ) : 0.0 );
				}
			...
			return ( get_option( 'woocommerce_discount_type', '' ) == 'flat' ) ? max( 0, $d[0] ) : min( 1.0, max( 0, ( 100.0 - round( $d[0], 2 ) ) / 100.0 ) );
		...
		public function filter_item_price( $price, $values ) {
			...
			if ( get_post_meta( $_product->id, "plus_discount_enabled", true ) != '' && get_post_meta( $_product->id, "plus_discount_enabled", true ) !== 'yes' ) {
				return $price;
			}
			...
			if ( empty( $this->discount_love ) || !isset( $this->discount_love[$this->get_actual_id( $_product )] )
				|| !isset( $this->discount_love[$this->get_actual_id( $_product )]['orig_price'] ) || !isset( $this->discount_love[$this->get_actual_id( $_product )]['coeff'] )
			) {
				$this->gather_discount_love();
			}
			$coeff = $this->discount_love[$this->get_actual_id( $_product )]['coeff'];
			if ( $coeff == 1.0 ) {
				return $price; // no price modification
			}
			$discprice = woocommerce_price( $_product->get_price() * $coeff );
			$oldprice = woocommerce_price( $this->discount_love[$this->get_actual_id( $_product )]['orig_price'] );
			...
			return "<span class='discount-info' title='" . sprintf( __( '%s%% Discounts Plus applied!', 'wc_plus_discount' ), round( ( 1.0 - $coeff ) * 100.0, 2 ) ) . "'>" .
		...
		public function filter_subtotal_price( $price, $values ) {
			...
			$coeff = $this->discount_love[$this->get_actual_id( $_product )]['coeff'];
			if ( ( get_option( 'woocommerce_discount_type', '' ) == 'flat' && $coeff == 0 ) || ( get_option( 'woocommerce_discount_type', '' ) == '' && $coeff == 1.0 ) ) {
				return $price; // no price modification
			}
			$new_css = esc_attr( get_option( 'woocommerce_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$plus_info = sprintf( __( 'With %s discount', 'wc_plus_discount' ), ( get_option( 'woocommerce_discount_type', '' ) == 'flat' ? get_woocommerce_currency_symbol() . $coeff : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );

			return "<span class='discount-info' title='$plus_info'>" .
			"<span>$price</span>" .
			"<span class='new-price' style='$new_css'> ($plus_info)</span></span>";
		...
		protected function gather_discount_love() {
			...
			$this->discount_love = array();

			if ( sizeof( $cart->cart_contents ) > 0 ) {
				foreach ( $cart->cart_contents as $cart_item_key => $values ) {
					$_product = $values['data'];
					$quantity = 0;
					if ( get_option( 'woocommerce_variations_separate', 'yes' ) == 'no' && $_product instanceof WC_Product_Variation && $_product->parent ) {
					...
					$this->discount_love[$this->get_actual_id( $_product )]['coeff'] = $this->get_discounted_coeff( $_product->id, $quantity );
					$this->discount_love[$this->get_actual_id( $_product )]['orig_price'] = $_product->get_price();
				}
			}
		...
		public function filter_subtotal_order_price( $price, $values, $order ) {
			...
			if ( get_post_meta( $values['product_id'], "plus_discount_enabled", true ) != '' && get_post_meta( $values['product_id'], "plus_discount_enabled", true ) !== 'yes' ) {
				return $price;
			}
			...
			$new_css = esc_attr( get_option( 'woocommerce_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$plus_info = sprintf( __( 'With %s discount', 'wc_plus_discount' ), ( $discount_type == 'flat' ? get_woocommerce_currency_symbol() . $coeff : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );
		...
		public function wdp_before_calculate( WC_Cart $cart ) {
			...
			if ( sizeof( $cart->cart_contents ) > 0 ) {
				...
				foreach ( $cart->cart_contents as $cart_item_key => $values ) {
					...
					if ( ( get_option( 'woocommerce_discount_type', '' ) == 'flat' ) ) {
						$row_base_price = max( 0, $_product->get_price() - ( $this->discount_love[$this->get_actual_id( $_product )]['coeff'] / $values['quantity'] ) );
					} else {
						$row_base_price = $_product->get_price() * $this->discount_love[$this->get_actual_id( $_product )]['coeff'];
					}
		...
		public function wdp_after_calculate( WC_Cart $cart ) {
			...
			if ( sizeof( $cart->cart_contents ) > 0 ) {
				foreach ( $cart->cart_contents as $cart_item_key => $values ) {
					...
					$values['data']->set_price( $this->discount_love[$this->get_actual_id( $_product )]['orig_price'] );
				}
			}
		...
		public function order_update_meta( $order_id ) {
			...
			update_post_meta( $order_id, "_woocommerce_discount_love", json_encode( $this->discount_love ) );
		...
		public function wdp_product_write_panel_tabs() {
			...
			echo '<li class="discounts_plus_tab discounts_plus_options"><a href="#discounts_plus_product_data" '.$style.'>' . __( 'Discounts Plus'. ($this->wdp_pro?'+':''), 'wc_plus_discount' ) . '</a></li>';
		...
		public function wdp_product_write_panels() {
			...
			<script type="text/javascript">
				jQuery( document ).ready( function () {
					...
					e.find( '#def_disc_criteria<?php echo $i; ?>' ).click( function () {
						...
						{
							alert( '<?php _e( 'Please fill in the current line before adding new line.', 'wc_plus_discount' ); ?>' );
						}*/
					} );
			...
			<div id="discounts_plus_product_data" class="panel woocommerce_options_panel">
				...
				<div class="options_group">
					<?php
					woocommerce_wp_checkbox( array( 'id' => 'plus_discount_enabled', 'value' => get_post_meta( $thepostid, 'plus_discount_enabled', true ) ? get_post_meta( $thepostid, 'plus_discount_enabled', true ) : 'yes', 'label' => __( 'Discounts Plus enabled', 'wc_plus_discount' ) ) );
					woocommerce_wp_textarea_input( array( 'id' => "plus_discount_text_info", 'label' => __( 'Discounts Plus special offer text in product description', 'wc_plus_discount' ), 'description' => __( 'Optionally enter Discounts Plus information that will be visible on the product page.', 'wc_plus_discount' ), 'desc_tip' => 'yes', 'class' => 'fullWidth' ) );
					?>
				</div>
				...
				for ( $i = 1;
					...
					<div class="options_group<?php echo $i; ?>">
						<a id="def_disc_criteria<?php echo $i; ?>" class="button-secondary"
						   href="#block<?php echo $i; ?>"><?php _e( 'Define discount criteria', 'wc_plus_discount' ); ?></a>
						<a id="delete_discount_line<?php echo $i; ?>" class="button-secondary"
						   href="#block<?php echo $i; ?>"><?php _e( 'Remove discount criteria', 'wc_plus_discount' ); ?></a>

						<div class="block<?php echo $i; ?> <?php echo ( $i % 2 == 0 ) ? 'even' : 'odd' ?>">
							<?php
							woocommerce_wp_text_input( array( 'id' => "plus_discount_quantity_$i", 'label' => __( 'Quantity (min.)', 'wc_plus_discount' ), 'type' => 'number', 'description' => __( 'Quantity on which the discount criteria will apply?', 'wc_plus_discount' ), 'custom_attributes' => array(
								'step' => '1',
								'min' => '1'
							) ) );
							if ( get_option( 'woocommerce_discount_type', '' ) == 'flat' ) {
								woocommerce_wp_text_input( array( 'id' => "plus_discount_discount_flat_$i", 'type' => 'number', 'label' => sprintf( __( 'Discount (%s)', 'wc_plus_discount' ), get_woocommerce_currency_symbol() ), 'description' => sprintf( __( 'Enter the flat discount in %s.', 'wc_plus_discount' ), get_woocommerce_currency_symbol() ), 'custom_attributes' => array(
									'step' => 'any',
									'min' => '0'
								) ) );
							} else {
								woocommerce_wp_text_input( array( 'id' => "plus_discount_discount_$i", 'type' => 'number', 'label' => __( 'Discount (%)', 'wc_plus_discount' ), 'description' => __( 'Discount percentage (Range: 0 to 100).', 'wc_plus_discount' ), 'custom_attributes' => array(
									'step' => 'any',
									'min' => '0',
									'max' => '100'
								) ) );
				...
				<div class="options_group<?php echo $this->opts; ?>">
					<a id="delete_discount_line<?php echo $this->opts; ?>" class="button-secondary"
					   href="#block<?php echo $this->opts; ?>"><?php _e( 'Remove discount criteria', 'wc_plus_discount' ); ?></a>
				</div>
		...
		public function wdp_enqueue_scripts() {
			...
			wp_register_style( 'woocommercediscounts_plus-style', plugins_url( 'css/style.css', __FILE__ ) );
			wp_enqueue_style( 'woocommercediscounts_plus-style' );
		...
		public function wdp_enqueue_scripts_admin() {
			...
			wp_register_style( 'woocommercediscounts_plus-style-admin', plugins_url( 'css/admin.css', __FILE__ ) );
			wp_enqueue_style( 'woocommercediscounts_plus-style-admin' );
			wp_enqueue_script( 'jquery' );
		...
		public function wdp_process_meta( $post_id ) {
			...
			if ( isset( $_POST['plus_discount_text_info'] ) ) update_post_meta( $post_id, 'plus_discount_text_info', stripslashes( $_POST['plus_discount_text_info'] ) );

			if ( isset( $_POST['plus_discount_enabled'] ) && $_POST['plus_discount_enabled'] == 'yes' ) {
				update_post_meta( $post_id, 'plus_discount_enabled', stripslashes( $_POST['plus_discount_enabled'] ) );
			} else {
				update_post_meta( $post_id, 'plus_discount_enabled', stripslashes( 'no' ) );
			}

			for ( $i = 1; $i < $this->opts; $i++ ) {
				if ( isset( $_POST["plus_discount_quantity_$i"] ) ) update_post_meta( $post_id, "plus_discount_quantity_$i", stripslashes( $_POST["plus_discount_quantity_$i"] ) );
				if ( ( get_option( 'woocommerce_discount_type', '' ) == 'flat' ) ) {
					if ( isset( $_POST["plus_discount_discount_flat_$i"] ) ) update_post_meta( $post_id, "plus_discount_discount_flat_$i", stripslashes( $_POST["plus_discount_discount_flat_$i"] ) );
				} else {
					if ( isset( $_POST["plus_discount_discount_$i"] ) ) update_post_meta( $post_id, "plus_discount_discount_$i", stripslashes( $_POST["plus_discount_discount_$i"] ) );
				}
			}
		...
		public function init_form_fields() {
			...
			// Define settings
			$this->fields['plus_discount'] = apply_filters( 'woocommerce_plus_discount_settings_fields', array(

				array( 'name' => __( 'Discounts Plus'.($this->wdp_pro?'+':''), 'wc_plus_discount' ), 'type' => 'title', 'desc' => __( 'The following options are specific to product Discounts Plus.', 'wc_plus_discount' ) . '<br /><br/><strong><i>' . __( 'After changing the settings, it is recommended to clear all sessions in WooCommerce &gt; System Status &gt; Tools.', 'wc_plus_discount' ) . '</i></strong>', 'id' => 'wdpplus_discounts_options' ),

				array(
					'name' => __( 'Discounts Plus globally enabled', 'wc_plus_discount' ),
					'id' => 'woocommerce_enable_plus_discounts',
					'desc' => __( '', 'wc_plus_discount' ),
					'std' => 'yes',
					'type' => 'checkbox',
					'default' => 'yes'
				),
				...
				array(
					'title' => __( 'Discount Type', 'wc_plus_discount' ),
					'id' => 'woocommerce_discount_type',
					'desc' => sprintf( __( 'Select the type of discount. Percentage Discount deducts amount of %% from price while Flat Discount deducts fixed amount in %s', 'wc_plus_discount' ), get_woocommerce_currency_symbol() ),
					'desc_tip' => true,
					'std' => 'yes',
					'type' => 'select',
					'css' => 'min-width:200px;',
					'class' => 'chosen_select',
					'options' => array(
						'' => __( 'Percentage Discount', 'wc_plus_discount' ),
						'flat' => __( 'Flat Discount', 'wc_plus_discount' )
					)
				),

				array(
					'name' => __( 'Treat product variations separately', 'wc_plus_discount' ),
					'id' => 'woocommerce_variations_separate',
					'desc' => __( 'You need to have this option unchecked to apply discounts to variations by shared quantity.', 'wc_plus_discount' ),
					'std' => 'yes',
					'type' => 'checkbox',
					'default' => 'yes'
				),
				...
				array(
					'name' => __( 'Remove any Discounts Plus if a coupon code is applied', 'wc_plus_discount' ),
				...
				array(
					'name' => __( 'Show discount information next to cart item price', 'wc_plus_discount' ),
					'id' => 'woocommerce_show_on_item',
					'desc' => __( 'Applies only to percentage discount.', 'wc_plus_discount' ),
					'std' => 'yes',
					'type' => 'checkbox',
					'default' => 'yes'
				),
				...
				array(
					'name' => __( 'Show discount information next to item subtotal price', 'wc_plus_discount' ),
				...
				array(
					'name' => __( 'Show discount information next to item subtotal price in order history', 'wc_plus_discount' ),
					'id' => 'woocommerce_show_on_order_subtotal',
					'desc' => __( 'Includes showing discount in order e-mails and invoices.', 'wc_plus_discount' ),
					'std' => 'yes',
					'type' => 'checkbox',
					'default' => 'yes'
				),
				...
				array(
					'name' => __( 'Optionally enter information about discounts visible on cart page.', 'wc_plus_discount' ),
				...
				array(
					'name' => __( 'Optionally change the CSS for old price on cart before discounting.', 'wc_plus_discount' ),
				...
				array(
					'name' => __( 'Optionally change the CSS for new price on cart after discounting.', 'wc_plus_discount' ),
				...
				array( 'type' => 'sectionend', 'id' => 'wdpplus_discounts_options' ),

				array(
					'desc' => 'If you find the WooCommerce Discounts Plus extension useful, please visit our online store for more <a target="_blank" href="http://shop.androidbubbles.com">premium products</a>.',
					'id' => 'woocommerce_plus_discount_notice_text',
					'type' => 'title'
				),

				array( 'type' => 'sectionend', 'id' => 'woocommerce_plus_discount_notice_text' )
			...
			$js = "
					...
					jQuery('<div class=\"wdp-guy\"><ul><li class=\"".($this->wdp_pro?'hide':'')."\">Go Premium!</li><li>Video Tutorial</li><li>Contact Developer</li></ul></div>').insertBefore($('form#mainform table.form-table.wdp-tbl'));
